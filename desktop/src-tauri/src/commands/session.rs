...
#[tauri::command]
pub async fn list_sessions(state: State<'_, Arc<ServerState>>) -> Result<String, String> {
    ...
    let list: Vec<serde_json::Value> = sessions
        ...
        .collect();
    Ok(serde_json::json!({ "sessions": list }).to_string())
}
...
pub fn list_available_shells() -> Vec<ShellInfo> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // Add WSL + Visual Studio shells before fragment discovery so dedup catches them
        shells.extend(vs_shells);
        ...
        // (vs_shells and wsl_shells were already added before fragment discovery)
    }
